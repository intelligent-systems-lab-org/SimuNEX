...
namespace ROS2
{
    ...
    internal class ROS2ForUnity
    {
        ...
        private Platform GetOS()
        {
            ...
            else if (Application.platform is RuntimePlatform.WindowsEditor or RuntimePlatform.WindowsPlayer)
            {
        ...
        private string GetOSName()
        {
            return GetOS() switch
            {
                ...
                _ => throw new NotSupportedException("Only Linux and Windows are supported"),
            };
        }
        ...
        {
            // Assuming your package is in the Packages folder
            string packagePath = Path.Combine(Application.dataPath, "..", "Packages", "com.intelsyslab.simunex", "Runtime", "Extensions");

            // Now handle Ros2ForUnity relative to packagePath
            if (InEditor())
            {
                packagePath = Path.Combine(packagePath, ros2ForUnityAssetFolderName);
            }

            if (InEditor())
            {
                packagePath = Path.Combine(packagePath, GetOSName());
            }

            if (InEditor() || GetOS() == Platform.Windows)
            {
                packagePath = Path.Combine(packagePath, "x86_64");
            }
        ...
        private void CheckROSVersionSourced()
        {
            ...
            List<string> supportedVersions = new() { "foxy", "galactic" };
            string supportedVersionsString = string.Join(", ", supportedVersions);
        ...
        internal ROS2ForUnity()
        {
            ...
            EditorApplication.quitting += DestroyROS2ForUnity;
#endif
            isInitialized = true;
        }
        ...
        public bool Ok()
        {
            return isInitialized && Ros2cs.Ok();
        }
...
#if UNITY_EDITOR
        private void EditorPlayStateChanged(PlayModeStateChange change)
        {
